...
2. **LOADING indicator**, because the data fetching runs after the page is rendered, so the data is not fetched instantly, therefore showing a loading state.
...
1. Clicked the link on 16:32:38(RT), a slight pause for 2s, then page loads showing 16:02:44(PT)
...
1. At first, it was 16:40:12 (Preview Time–PT), and real-time when reloading is 16:40:25(Real-Time–RT) and 16:40:29(Real-Time–RT). In those 2 reload, Preview Time (PT) did not change.
...
That's all, I'll eventually make blog about each method pros and cons, and also some use cases. So stay tuned!
