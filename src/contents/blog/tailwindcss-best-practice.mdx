...
@layer base {
  ...
  h4 {
    @apply font-bold font-primary text-lg;
  }
      ...
      <ul className='flex items-center justify-between px-8 py-4'>
        ...
        <ul className='flex items-center justify-between space-x-4'>
          {links.map(({ href, label }) => (
            <li key={`${href}${label}`}>
              <Link href={href}>
                <a className='hover:text-green-400 text-white'>{label}</a>
              </Link>
            </li>
          ))}
        </ul>
...
export default function Tag({
  ...
  return (
    <button
      className={clsx(
        className,
        'font-medium inline-block px-1.5 py-0.5 rounded-md transition-colors',
        'bg-gray-100 disabled:bg-gray-200 disabled:text-gray-300 hover:text-black text-gray-700',
        'disabled:cursor-not-allowed focus-visible:ring focus-visible:ring-primary-300 focus:outline-none'
      )}
      {...rest}
    >
      {children}
